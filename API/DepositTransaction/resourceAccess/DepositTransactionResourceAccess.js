...
const tableName = "deposittransaction";
    ...
    DB.schema.dropTableIfExists(`${tableName}`).then(() => {
      DB.schema
        .createTable(`${tableName}`, function (table) {
          table.increments('depositTransactionId').primary();
          ...
          table.string('status').defaultTo(DEPOSIT_TRX_STATUS.NEW);
          ...
          table.string('hinhxacnhan', 2000).defaultTo('');
          timestamps(table);
          table.index('userId');
